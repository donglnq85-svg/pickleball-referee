...
export function historyDetail(m){
  ...
  return `<section class="ref-detail-result"><h2>Đội ${winner} thắng trận</h2><div class="ref-detail-teams"><span>${names(m,'A')}<b>${m.gamesWon.A}</b></span><span>${names(m,'B')}<b>${m.gamesWon.B}</b></span></div></section>
    <section class="ref-detail-section"><h3>Kết quả từng game</h3>${m.completedGames.map(g=>`<div class="ref-detail-row">Game ${g.gameNumber}<strong>${g.points.A} – ${g.points.B}</strong><small>Đội ${g.winner} thắng</small></div>`).join('')}</section>
    ...
    <section class="ref-detail-section"><h3>Diễn biến trận · ${events.length} sự kiện</h3><ol class="ref-event-log">${events.map(e=>`<li><span>${escape(eventName(e))}</span>${e.after?.currentGamePoints?`<small>Game ${e.after.game} · ${e.after.currentGamePoints.A} – ${e.after.currentGamePoints.B} · xướng ${scoreCall(e.after)}</small>`:''}</li>`).join('')}</ol></section>`;
}
