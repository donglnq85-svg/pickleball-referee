...
function renderTournament(){screen='tournament';const t=selected(),version=t.rulesVersions.find(v=>v.id===t.activeRulesVersionId);
  ...
  <section class="t2-section"><h2>Bắt đầu nhiệm vụ</h2>${t.assignments.filter(a=>a.status==='assigned').map(a=>button(`Bắt đầu · ${escape(a.label)}`,`start:${a.id}`)).join('')||'<p class="v1-muted">Chưa có phân công mới.</p>'}</section>`,button('Danh sách giải','list',true));
}
