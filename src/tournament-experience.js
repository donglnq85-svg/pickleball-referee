...
function standingsTable(t,g,compact){const rows=groupRows(t,g);return `<section class="tx-standings ${compact?'compact':''}"><h3>${escape(g.label)} — ${escape(g.category||'Chưa có nội dung')}</h3><div class="tx-table-head"><span>Cặp VĐV</span><span>Trận</span><span>Thắng</span><span>Thua</span><span>Hiệu số</span><span>Điểm</span></div>${rows.map((row,index)=>`<div class="tx-table-row"><i>${row.rank||index+1}</i><b>${entryPlayers(t,row.entry.id).map(escape).join(' / ')}</b><span>${row.played}</span><span>${row.wins}</span><span>${row.losses}</span><span>${row.pointsFor-row.pointsAgainst>=0?'+':''}${row.pointsFor-row.pointsAgainst}</span><strong>—</strong></div>`).join('')}</section>`}
