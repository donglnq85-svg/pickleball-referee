...
const tournamentStatus=t=>t.status==='active'?'Đang diễn ra':new Date(t.endsAt||t.startsAt||0)<new Date()?'Đã kết thúc':'Sắp diễn ra';
...
function renderRules(){
  ...
  shell(`${detailHeader(t,'rules')}<div class="tx-note">Thiết lập thể thức tính điểm theo từng vòng. Các trận đấu sử dụng cấu hình hiện đang áp dụng của giải.</div><div class="tx-section-head"><h3>Danh sách vòng thi đấu</h3><button data-tx="configureRules">+ Thêm vòng</button></div><div class="tx-round-list">${rounds.map((round,index)=>`<section class="tx-round-card"><div><b>${round}</b><small>Áp dụng theo cài đặt hiện hành</small></div><button aria-label="Chỉnh sửa" data-tx="configureRules">✎</button><dl><div><dt>Số game</dt><dd>${format?.sets||'Chưa có'}</dd></div><div><dt>Điểm mỗi game</dt><dd>${format?.points||'Chưa có'}</dd></div><div><dt>Cách biệt thắng</dt><dd>${format?format.rule==='touch'?'Chạm điểm':'Cách 2 điểm':'Chưa có'}</dd></div><div><dt>Giới hạn điểm</dt><dd>${format?.cap?`Tối đa ${format.cap}`:format?'Không giới hạn':'Chưa có'}</dd></div></dl></section>`).join('')}</div><button class="tx-secondary">Sao chép từ giải đấu khác</button>`,{title:'Thể thức tính điểm',subtitle:t.name,back:'info'});
}
...
function groupRows(t,g){
  const entries=groupEntries(t,g),stats=new Map(entries.map(e=>[e.id,{entry:e,played:0,wins:0,losses:0,pointsFor:0,pointsAgainst:0}]));
  ...
  return [...stats.values()].sort((a,b)=>b.wins-a.wins||(b.pointsFor-b.pointsAgainst)-(a.pointsFor-a.pointsAgainst));
}
...
function renderGroup(){
  ...
  if(groupTab==='athletes')body=`<div class="tx-section-head"><h3>Danh sách cặp VĐV (${entries.length} cặp)</h3><button data-tx="addGroup">✎ Thêm / Chỉnh sửa</button></div><div class="tx-athlete-list">${entries.map((entry,index)=>`<div><span>${index+1}</span><b>${entryPlayers(t,entry.id).map(escape).join('<br>')}</b><strong>›</strong></div>`).join('')}</div><section class="tx-format-card"><div class="tx-section-head"><h3>Thông tin thể thức thi đấu</h3><button data-tx="rules">Xem chi tiết ›</button></div><p>Áp dụng theo cài đặt của giải đấu cho ${escape(g.formatLabel||'Vòng bảng')}.</p>${formatSummary(t)}</section><div class="tx-quick-stats"><div><b>${entries.length}</b><small>Cặp VĐV</small></div><div><b>${matches.length}</b><small>Trận đấu</small></div><div><b>${matches.filter(m=>resultFor(t,m)?.status==='CONFIRMED').length}</b><small>Đã hoàn thành</small></div><div><b>${matches.filter(m=>!m.matchSessionId).length}</b><small>Chưa thi đấu</small></div></div><button class="tx-secondary">Xuất lịch thi đấu</button><button class="tx-primary" data-tx="beginGroup">Bắt đầu thi đấu</button>`;
  if(groupTab==='schedule')body=`<div class="tx-day-tabs"><button class="active">${fmtDate(t.startsAt)}</button><button>Xem theo sân</button></div><div class="tx-section-head"><h3>Danh sách trận đấu (${matches.length} trận)</h3></div><div class="tx-match-list">${matches.map((m,index)=>matchRow(t,m,index,true)).join('')||'<p class="tx-muted">Chưa có lịch thi đấu.</p>'}</div><section class="tx-rule-line">Thể thức: ${formatInline(t)}</section><button class="tx-secondary">Xuất lịch thi đấu</button><button class="tx-primary" data-tx="beginGroup">Vào trận đang diễn ra</button>`;
  if(groupTab==='results')body=`<div class="tx-day-tabs"><button class="active">${fmtDate(t.startsAt)}</button><button>Xem theo sân</button></div>${standingsTable(t,g,true)}<div class="tx-section-head"><h3>Kết quả thi đấu (${matches.length} trận)</h3><small>Cập nhật từ kết quả đã xác nhận</small></div><div class="tx-match-list">${matches.map((m,index)=>matchRow(t,m,index,false)).join('')}</div>`;
  if(groupTab==='standings')body=`${standingsTable(t,g,false)}<section class="tx-ranking-note"><h3>Cách xếp hạng</h3><p>Chỉ sử dụng tiêu chí đã được cấu hình trong giải. Khi chưa đủ quy định, hệ thống giữ trạng thái cần xác nhận và không tự đoán thứ hạng.</p></section><button class="tx-secondary">Xuất bảng xếp hạng</button>`;
  shell(`${groupHeader(t,g)}${body}`,{title:g.label,subtitle:t.name,back:'groups',action:`<button class="tx-icon" aria-label="Tùy chọn">${glyph('more')}</button>`});
}
...
function standingsTable(t,g,compact){const rows=groupRows(t,g);return `<section class="tx-standings ${compact?'compact':''}"><h3>${escape(g.label)} — ${escape(g.category||'Chưa có nội dung')}</h3><div class="tx-table-head"><span>Cặp VĐV</span><span>Trận</span><span>Thắng</span><span>Thua</span><span>Hiệu số</span><span>Điểm</span></div>${rows.map((row,index)=>`<div class="tx-table-row"><i>${index+1}</i><b>${entryPlayers(t,row.entry.id).map(escape).join(' / ')}</b><span>${row.played}</span><span>${row.wins}</span><span>${row.losses}</span><span>${row.pointsFor-row.pointsAgainst>=0?'+':''}${row.pointsFor-row.pointsAgainst}</span><strong>${row.wins}</strong></div>`).join('')}</section>`}
