...
test('Gate 8 projects immutable Tournament, Work, Match, Group and Quick Match history with exact version semantics',()=>{
  ...
  assert.deepEqual(record.reports.versions.map(item=>item.status),['OUTDATED','SENT']);assert.ok(record.significantEvents.some(item=>item.type==='canonicalResultVersionCreated'&&item.label.includes('Correction')));assert.ok(record.significantEvents.some(item=>item.type==='reportGenerated'&&item.detail.status==='NEEDS_RESEND'));assert.ok(record.fullTimeline.length>record.significantEvents.length);
  ...
  assert.equal(workRecord.shiftCompletionSnapshot.id,completion.id);assert.equal(workRecord.handoverSnapshots[0].id,handover.id);assert.equal(workRecord.handoverSnapshots[0].outstanding[0].issueId,issue.id);assert.equal(workRecord.outstanding.outstanding.find(item=>item.issueId===issue.id).kind,'OPERATIONAL_ISSUE');assert.ok(workRecordDetail(workRecord,projection).includes('HANDED_OVER không đồng nghĩa RESOLVED'));
  assert.equal(groupRecord.snapshots.at(-1).id,groupV2.id);assert.equal(groupRecord.snapshots.at(-1).rankingRulesVersion.id,ranking.id);assert.deepEqual(groupRecord.snapshots.at(-1).resultVersionIds,[resultV2.id]);assert.ok(groupRecordDetail(groupRecord).includes(resultV2.id));
